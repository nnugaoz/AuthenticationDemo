...
using System.Data;
using System.Web;
public class T_Weight : IHttpHandler
{
    ...
    {
        DataTable lDT = null;
        T_WeightDao lDao = new T_WeightDao();
        lDT = lDao.Select();
    ...
    private void SelectPage(HttpContext context)
    {
        ...
        string Gross = context.Request.Params["Gross"].ToString();
        string Tare = context.Request.Params["Tare"].ToString();
        string Net = context.Request.Params["Net"].ToString();
        string GrossTime = context.Request.Params["GrossTime"].ToString();
        string TareTime = context.Request.Params["TareTime"].ToString();
        string NetTime = context.Request.Params["NetTime"].ToString();
        string EditMan = context.Request.Params["EditMan"].ToString();
        string EditDate = context.Request.Params["EditDate"].ToString();
        lDT = lDao.SelectPage(CarNO, ProductName, Gross, Tare, Net, GrossTime, TareTime, NetTime, EditMan, EditDate, BeginIndex, EndIndex);
        context.Response.Write(JsonConvert.SerializeObject(lDT));
    }
    ...
    {
        T_WeightModel lModel = new T_WeightModel();
        lModel.ID = context.Request.Params["ID"].ToString();
        lModel.CarNO = context.Request.Params["CarNO"].ToString();
        lModel.ProductName = context.Request.Params["ProductName"].ToString();
        lModel.Gross = Convert.ToDecimal(context.Request.Params["Gross"].ToString());
        lModel.Tare = Convert.ToDecimal(context.Request.Params["Tare"].ToString());
        lModel.Net = Convert.ToDecimal(context.Request.Params["Net"].ToString());
        lModel.GrossTime = context.Request.Params["GrossTime"].ToString();
        lModel.TareTime = context.Request.Params["TareTime"].ToString();
        lModel.NetTime = context.Request.Params["NetTime"].ToString();
