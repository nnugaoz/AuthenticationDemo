!function(window) {
    function ActionItemsResolver() {
        ...
        function resolveFromPageReportContent(content) {
            return $(content).find("a").map(function(index, item) {
                ...
                return null !== item;
            }).get()).concat($(content).find("span, img").map(function(index, item) {
                return resolvePageReportElementAction($(item));
            }).filter(function(item) {
                return null !== item;
            }).get());
        ...
        function processToolTips() {
            ...
            if (allAreas.length > 0) {
                ...
                allAreas.each(function() {
                    var contentTooltip = $("<div/>"), map = $(this).parent(), lastInd = $(this).attr("data-category-id").lastIndexOf("-"), categoryId = $(this).attr("data-category-id").substring(0, lastInd), categoryLabel = ($(this).attr("data-category-id").substring(lastInd + 1), 
                    $(this).attr("data-category-label"));
                    categoryLabel && contentTooltip.append($("<div/>").text(categoryLabel).addClass("tooltip-label")), 
                    map.find("area[data-category-id^=" + categoryId + "-]").each(function() {
                    ...
                    }), categoryId = map.attr("name") + categoryId, tooltips[categoryId] || (tooltips[categoryId] = contentTooltip);
                }), $("#reportContainer, #continuousView").on("mouseenter", "area[data-point-tooltip]", function(e) {
                    e.stopPropagation();
                    var mapName = $(this).parent().attr("name"), lastInd = $(this).attr("data-category-id").lastIndexOf("-"), currentCategoryId = mapName + $(this).attr("data-category-id").substring(0, lastInd), content = tooltips[currentCategoryId];
    ...
    function ViewerImpl(services, options, viewModels) {
        ...
        var documentReadySubscribe, reportService = services.reportService, resourceManager = services.resourceManager, templates = services.templates, viewerModel = viewModels.viewerModel, viewModel = viewModels.viewerViewModel, viewer = {
            ...
            reportService: function() {
                return arguments.length > 0 && (options.reportService = arguments[0], reportService = ArReportService(options.reportService, resourceManager), 
    ...
    function createViewer(options) {
        var baseUri = options.baseUri || ".", resourceManager = ResourceManager(), reportService = ArReportService(options.reportService, resourceManager), templates = Templates(baseUri, resourceManager), browserSpecific = BrowserSpecific(), printingService = PrintingService(resourceManager, browserSpecific), services = {
