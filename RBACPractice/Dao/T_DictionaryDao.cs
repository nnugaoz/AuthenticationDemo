...
public class T_DictionaryDao
{
    ...
    public static string SelectByTypeSQL()
    {
        string lSQL = "";
        lSQL += " SELECT";
        lSQL += " T_Dic.ID";
        lSQL += ",T_Dic.DKey";
        lSQL += ",T_Dic.DValue";
        lSQL += ",T_Dic_Typ.DTKey";
        lSQL += ",T_Dic_Typ.DTValue";
        lSQL += " FROM T_Dictionary T_Dic";
        lSQL += " LEFT JOIN T_Dictionary_Type T_Dic_Typ ON T_Dic.DTID=T_Dic_Typ.ID";
        lSQL += " WHERE T_Dic_Typ.DTKey=@DTKey";
        return lSQL;
    }
    ...
    internal DataTable SelectByType(string DTKey)
    {
        string lSQL = SelectByTypeSQL();
        DataTable lDT = null;
        List<SqlParameter> lParams = new List<SqlParameter>();
        lParams.Add(new SqlParameter("@DTKey", DTKey));
        MsSqlHelper lMSSqlHelper = new MsSqlHelper();
        lDT = lMSSqlHelper.GetDataTable(lSQL, lParams.ToArray());
        return lDT;
    }
